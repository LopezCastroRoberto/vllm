...
def _chunk_delta_h_warmup_inputs(
    *,
    k_dtype: torch.dtype,
    v_dtype: torch.dtype,
    w_dtype: torch.dtype,
    ...
    return {
        "k": TritonWarmupTensor(
            k_dtype, shape=(b, block_t, num_k_heads, qk_head_dim)
        ),
        "v": TritonWarmupTensor(
            v_dtype, shape=(b, block_t, num_heads, v_head_dim)
        ),
        "w": TritonWarmupTensor(
            w_dtype, shape=(b, block_t, num_heads, qk_head_dim)
        ),
        "v_new": (
            TritonWarmupTensor(
                v_dtype, shape=(b, block_t, num_heads, v_head_dim)
        ...
        "g": (
            TritonWarmupTensor(gk_dtype, shape=(b, block_t, num_heads))
            if use_g
            else None
        ),
        "gk": (
            TritonWarmupTensor(
                gk_dtype, shape=(b, block_t, num_heads, qk_head_dim)
            )
            if use_gk
            else None
        ),
        "h": TritonWarmupTensor(
            k_dtype, shape=(b, nt, num_heads, v_head_dim, qk_head_dim)
        ),
        "h0": (
            TritonWarmupTensor(
                h0_dtype, shape=(n, num_heads, v_head_dim, qk_head_dim)
            )
        ...
        "ht": (
            TritonWarmupTensor(
                ht_dtype, shape=(n, num_heads, v_head_dim, qk_head_dim)
            )
...
@simple_triton_jit_kernel(warmup_inputs=_chunk_delta_h_warmup_inputs)
@triton.heuristics(
    {
...
@chunk_gated_delta_rule_fwd_kernel_h_blockdim64.launcher
def _CHUNK_GATED_DELTA_RULE_FWD_H_KERNEL(
    ...
    *, chunk_size, use_exp2,
    ...
    return triton_cdiv_grid((v_head_dim, "BV"), n * num_heads), dict(
        T=t, H=num_heads, Hg=hg, K=qk_head_dim, V=v_head_dim,
        BT=chunk_size, USE_EXP2=use_exp2,
    )
