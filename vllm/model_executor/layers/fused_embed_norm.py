...
class FusedEmbedNormKernel(DeclarativeTritonJitKernel):
    ...
    def warmup_cases(
        ...
        *,
        ...
        output = TritonWarmupTensor(table_dtype, shape=(1, hidden_size))
        return dict(
            ids_dtype=compile_key(ids_dtype),
            table_dtype=compile_key(table_dtype),
            table_stride=compile_key(triton_scalar_specialization_rep(table_stride)),
            hidden_size=compile_key(hidden_size),
            block_size=compile_key(triton.next_power_of_2(hidden_size)),
            num_warps=compile_key(
                min(32, max(4, triton.next_power_of_2(hidden_size) // 512))
            ),
            ids=TritonWarmupTensor(ids_dtype),
            table=table,
            chain_w=(
                TritonWarmupTensor(table_dtype, shape=(hidden_size,))
                if has_norm
                else table
            ),
            eps=0.0,
...
def fused_embed_norm(
    ...
    normed = torch.empty_like(out) if has_norm else out
    ...
    return (out, normed) if has_norm else out
...
class FusedEmbedEhNormKernel(DeclarativeTritonJitKernel):
    ...
    def warmup_cases(
        ...
        *,
        ...
        return dict(
            ids_dtype=compile_key(ids_dtype),
            table_dtype=compile_key(table_dtype),
            hidden_dtype=compile_key(hidden_dtype),
            hidden_size=compile_key(hidden_size),
            block_size=compile_key(triton.next_power_of_2(hidden_size)),
            table_stride=compile_key(triton_scalar_specialization_rep(hidden_size)),
            hidden_stride=compile_key(triton_scalar_specialization_rep(hidden_size)),
            output_stride=compile_key(
                triton_scalar_specialization_rep(2 * hidden_size)
            ),
            pos=TritonWarmupTensor(torch.int64),
            ids=TritonWarmupTensor(ids_dtype),
            table=TritonWarmupTensor(table_dtype, shape=(1, hidden_size)),
            prev=hidden,
            enorm_w=hidden,
            hnorm_w=hidden,
            eps=0.0,
            out=TritonWarmupTensor(hidden_dtype, shape=(1, 2 * hidden_size)),
        )
