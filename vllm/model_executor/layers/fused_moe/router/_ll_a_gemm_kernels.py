...
@dsl_user_op
def atom_add_global_f32(addr, val, *, loc=None, ip=None):
    """atom.global.add.f32 — global memory fp32 atomic add."""
    i32 = _ir.IntegerType.get_signless(32)
    i64 = _ir.IntegerType.get_signless(64)
    addr_int = addr.toint(loc=loc, ip=ip)
    val_ir = val.ir_value(loc=loc, ip=ip)
    _llvm.inline_asm(i32, [addr_int, val_ir],
        "atom.global.add.f32 $0, [$1], $2; mov.u32 $3, 0;",
        "=f,l,f,=r", has_side_effects=True, loc=loc, ip=ip)
...
class LLAGemm:
            ...
            for ei in cutlass.range_constexpr(elems_per_thread):
                ...
                if global_m < cute.size(mC, mode=[0]):
                    if global_n < N_global:
                        ...
                        out_r[0] = (total * scale).to(self.out_dtype)
                        out_t[0] = out_r[0]
    ...
    @cute.jit
    def call_splitk(self, mA: cute.Tensor, mB: cute.Tensor, mC: cute.Tensor,
                    stream: CUstream, scale: float = 1.0):
        bM, bN, bK = self.tile_m, self.tile_n, self.tile_k
        copy_bits = 128
        ...
        sC_layout = self._make_smem_layout_C(mC.element_type, copy_bits, (bM, bN))
        atom_g2s = cute.make_copy_atom(cute.nvgpu.cpasync.CopyG2SOp(cache_mode=cute.nvgpu.cpasync.LoadCacheMode.GLOBAL), mA.element_type, num_bits_per_copy=copy_bits)
        ...
        atom_s2g = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), mC.element_type, num_bits_per_copy=copy_bits)
        c_copy_elems = copy_bits // mC.element_type.width
        cn_threads = bN // c_copy_elems
        tiled_copy_C = cute.make_tiled_copy_tv(atom_s2g, cute.make_layout((self.num_mma_threads // cn_threads, cn_threads), stride=(cn_threads, 1)), cute.make_layout((1, c_copy_elems)))
        op = cute.nvgpu.warp.MmaF16BF16Op(self.ab_dtype, self.acc_dtype, self.mma_shape)
        perm_mnk = (self.atom_layout[0] * self.mma_shape[0], self.atom_layout[1] * self.mma_shape[1] * (self.tile_n // 8), self.atom_layout[2] * self.mma_shape[2])
        tiled_mma = cute.make_tiled_mma(op, cute.make_layout(self.atom_layout), permutation_mnk=perm_mnk)
        grid_m = cute.ceil_div(cute.size(mC, mode=[0]), bM)
        grid_n = cute.ceil_div(cute.size(mC, mode=[1]), bN)
        self.kernel_splitk(mA, mB, mC, scale, sA_layout, sB_layout, sC_layout,
            tiled_copy_A, tiled_copy_B, tiled_copy_C, tiled_mma).launch(
    ...
    @cute.kernel
    def kernel_splitk(self, mA, mB, mC, scale: cutlass.Float32,
        sA_layout: cute.ComposedLayout, sB_layout: cute.ComposedLayout, sC_layout: cute.Layout,
        tiled_copy_A: cute.TiledCopy, tiled_copy_B: cute.TiledCopy,
        tiled_copy_C: cute.TiledCopy, tiled_mma: cute.TiledMma):
        bM, bN, bK = self.tile_m, self.tile_n, self.tile_k
        num_stages = self.num_stages
        tidx, _, _ = cute.arch.thread_idx()
        ...
        class SharedStorage:
            ...
            c: cute.struct.Align[cute.struct.MemRange[mC.element_type, cute.cosize(sC_layout)], 16]
            mbar: cute.struct.Align[cute.struct.MemRange[cutlass.Int64, num_stages * 2], 8]
        smem = cutlass.utils.SmemAllocator()
        storage_ptr = smem.allocate(SharedStorage.size_in_bytes(), byte_alignment=16)
        storage = SharedStorage(storage_ptr)
        sA = storage.a.get_tensor(sA_layout)
        ...
        mainloop_pipeline = pipeline.PipelineCpAsync.create(barrier_storage=storage.mbar.data_ptr(), num_stages=num_stages, producer_group=producer_group, consumer_group=consumer_group)
        ...
        if is_dma:
            cute.arch.setmaxregister_decrease(40)
            thr_A = tiled_copy_A.get_slice(dma_tidx); thr_B = tiled_copy_B.get_slice(dma_tidx)
            tAgA = thr_A.partition_S(gA); tAsA = thr_A.partition_D(sA)
            tBgB = thr_B.partition_S(gB); tBsB = thr_B.partition_D(sB)
            tAcA = thr_A.partition_S(cA); tBcB = thr_B.partition_S(cB)
            tApA = cute.make_rmem_tensor(cute.make_layout((tAgA.shape[0][1], cute.size(tAgA, mode=[1]), cute.size(tAgA, mode=[2])), stride=(cute.size(tAgA, mode=[1]), 1, 0)), cutlass.Boolean)
            for rv in range(tApA.shape[0]):
                for m in range(tApA.shape[1]):
                    tApA[rv, m, 0] = cute.elem_less(tAcA[(0, rv), m, 0, 0][0], mA.shape[0])
            tBpB = cute.make_rmem_tensor(cute.make_layout((tBgB.shape[0][1], cute.size(tBgB, mode=[1]), cute.size(tBgB, mode=[2])), stride=(cute.size(tBgB, mode=[1]), 1, 0)), cutlass.Boolean)
            for rv in range(tBpB.shape[0]):
                for n in range(tBpB.shape[1]):
                    tBpB[rv, n, 0] = cute.elem_less(tBcB[(0, rv), n, 0, 0][0], mB.shape[0])
            producer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, num_stages)
            # First tile: PDL overlap — prefetch B (weight) before wait, load A (activation) after
            mainloop_pipeline.producer_acquire(producer_state)
            cute.copy(tiled_copy_B, tBgB[None, None, None, k_start], tBsB[None, None, None, producer_state.index], pred=tBpB)
            cute.arch.griddepcontrol_wait()
            cute.copy(tiled_copy_A, tAgA[None, None, None, k_start], tAsA[None, None, None, producer_state.index], pred=tApA)
            mainloop_pipeline.producer_commit(producer_state)
            producer_state.advance()
            ...
            # Remaining tiles: update predicate for K-bounds on partial tiles
            for local_k in range(1, my_tiles):
                ...
                for rv in range(tApA.shape[0]):
                    ...
                    for m in range(tApA.shape[1]):
                        tApA[rv, m, 0] = cute.elem_less(tAcA[(0, rv), m, 0, 0][0], mA.shape[0]) & k_valid
                    for n in range(tBpB.shape[1]):
                        tBpB[rv, n, 0] = cute.elem_less(tBcB[(0, rv), n, 0, 0][0], mB.shape[0]) & k_valid
                mainloop_pipeline.producer_acquire(producer_state)
                cute.copy(tiled_copy_A, tAgA[None, None, None, k_tile], tAsA[None, None, None, producer_state.index], pred=tApA)
                cute.copy(tiled_copy_B, tBgB[None, None, None, k_tile], tBsB[None, None, None, producer_state.index], pred=tBpB)
                mainloop_pipeline.producer_commit(producer_state)
                producer_state.advance()
            mainloop_pipeline.producer_tail(producer_state)
        else:
            ...
            tCrA = tiled_mma.make_fragment_A(tCsA[None, None, None, 0])
            tCrB = tiled_mma.make_fragment_B(tCsB[None, None, None, 0])
            tCrC = tiled_mma.make_fragment_C(tCgC); tCrC.fill(0.0)
            atom_s2r_A = cute.make_copy_atom(cute.nvgpu.warp.LdMatrix8x8x16bOp(False, 4), mA.element_type)
            atom_s2r_B = cute.make_copy_atom(cute.nvgpu.warp.LdMatrix8x8x16bOp(False, 4), mB.element_type)
            tiled_s2r_A = cute.make_tiled_copy_A(atom_s2r_A, tiled_mma)
            tiled_s2r_B = cute.make_tiled_copy_B(atom_s2r_B, tiled_mma)
            thr_s2r_A = tiled_s2r_A.get_slice(lane_id); thr_s2r_B = tiled_s2r_B.get_slice(lane_id)
            tCsA_v = thr_s2r_A.partition_S(sA); tCrA_v = thr_s2r_A.retile(tCrA)
            tCsB_v = thr_s2r_B.partition_S(sB); tCrB_v = thr_s2r_B.retile(tCrB)
            num_k_block = cute.size(tCrA, mode=[2])
            K_PER_WARP: cutlass.Constexpr = num_k_block // NUM_MMA_WARPS
            ...
            for local_k in range(my_tiles):
                ...
                if not self.is_fp8:
                    for ki in cutlass.range(K_PER_WARP, unroll_full=True):
                        k_block = ki * NUM_MMA_WARPS + mma_warp_idx
                        cute.copy(tiled_s2r_A, tCsA_p[None, None, k_block], tCrA_v[None, None, 0])
                        cute.copy(tiled_s2r_B, tCsB_p[None, None, k_block], tCrB_v[None, None, 0])
                        cute.gemm(tiled_mma, tCrC, tCrA[None, None, 0], tCrB[None, None, 0], tCrC)
                else:
                    c0=tCrC[0];c1=tCrC[1];c2=tCrC[2];c3=tCrC[3];c4=tCrC[4];c5=tCrC[5];c6=tCrC[6];c7=tCrC[7]
                    a_s=tCrA[None,None,0]; b_s=tCrB[None,None,0]
                    for ki in cutlass.range(K_PER_WARP, unroll_full=True):
                        k_block = ki * NUM_MMA_WARPS + mma_warp_idx
                        cute.copy(tiled_s2r_A, tCsA_p[None, None, k_block], tCrA_v[None, None, 0])
                        cute.copy(tiled_s2r_B, tCsB_p[None, None, k_block], tCrB_v[None, None, 0])
                        c0,c1,c2,c3,c4,c5,c6,c7 = fused_fp8_mma_2n(c0,c1,c2,c3,c4,c5,c6,c7,a_s[0],a_s[1],a_s[2],a_s[3],a_s[4],a_s[5],a_s[6],a_s[7],b_s[0],b_s[1],b_s[2],b_s[3],b_s[4],b_s[5],b_s[6],b_s[7])
                    tCrC[0]=c0;tCrC[1]=c1;tCrC[2]=c2;tCrC[3]=c3;tCrC[4]=c4;tCrC[5]=c5;tCrC[6]=c6;tCrC[7]=c7
                mainloop_pipeline.consumer_release(consumer_state)
                consumer_state.advance()

            # === CLUSTER REDUCTION EPILOGUE ===
            # Step 1: reduce MMA warps → per-thread fp32 partial in smem
            smem_red_ptr = cute.arch.alloc_smem(cutlass.Float32, bM * bN * NUM_MMA_WARPS, alignment=16)
            smem_warp = cute.make_tensor(smem_red_ptr + mma_warp_idx * bM * bN, cute.make_layout((bM, bN), stride=(bN, 1)))
            tCsC_partial = thr_mma.partition_C(smem_warp)
            cute.autovec_copy(tCrC, tCsC_partial)
            cute.arch.sync_threads()
            ...
            # Reduce across warps, scale, store to smem partials buffer
            partial_buf = cute.arch.alloc_smem(cutlass.Float32, bM * bN * self.split_k, alignment=16)
            ...
            # Step 2: send partials to all peer CTAs via st.shared::cluster
            for ei in cutlass.range_constexpr(elems_per_thread):
                idx = ei * self.num_mma_threads + mma_tidx
            ...
            # Step 3: cluster barrier
            cluster_arrive_relaxed()
            cluster_wait()

            # Signal next kernel AFTER cluster reduction is complete
            if mma_tidx == 0:
                cute.arch.griddepcontrol_launch_dependents()
            cute.arch.sync_threads()

            # Step 4: local reduction + global output write
            for ei in cutlass.range_constexpr(elems_per_thread):
                idx = ei * self.num_mma_threads + mma_tidx
                m = idx // bN
                n = idx % bN
    ...
    @cute.jit
    def call_mpar(self, mA: cute.Tensor, mB: cute.Tensor, mC: cute.Tensor,
                  stream: CUstream, scale: float = 1.0):
        """M-parallel warp dispatch: 4 MMA warps handle 4 m16 sub-tiles = tile_m=64."""
        bM_total = 16 * self.num_mma_warps  # 64
        bN, bK = self.tile_n, self.tile_k
        copy_bits = 128

        # smem layout for the FULL tile_m (64 rows)
        sA_layout = self._make_smem_layout_AB(mA.element_type, copy_bits, (bM_total, bK, self.num_stages))
        sB_layout = self._make_smem_layout_AB(mB.element_type, copy_bits, (bN, bK, self.num_stages))
        # Per-warp smem layout (16 rows) for MMA partitioning
        sA_warp_layout = self._make_smem_layout_AB(mA.element_type, copy_bits, (16, bK, self.num_stages))
        ...
        grid_m = cute.ceil_div(cute.size(mC, mode=[0]), bM_total)
        grid_n = cute.ceil_div(cute.size(mC, mode=[1]), bN)
        scale_val = cutlass.Float32(scale)

        self.kernel_mpar(mA, mB, mC, scale_val, sA_layout, sB_layout, sA_warp_layout,
            tiled_copy_A, tiled_copy_B, tiled_copy_C, tiled_mma).launch(
            grid=[cute.size(grid_m), cute.size(grid_n), 1],
            block=[self.num_threads, 1, 1],
            stream=stream, use_pdl=True)
    ...
    @cute.kernel
    def kernel_mpar(self, mA, mB, mC, scale: cutlass.Float32,
        sA_layout: cute.ComposedLayout, sB_layout: cute.ComposedLayout,
        sA_warp_layout: cute.ComposedLayout,
        tiled_copy_A: cute.TiledCopy, tiled_copy_B: cute.TiledCopy,
        tiled_copy_C: cute.TiledCopy, tiled_mma: cute.TiledMma):
        """M-parallel warp kernel: 4 MMA warps, each handles own m16 sub-tile."""
        bN, bK = self.tile_n, self.tile_k
        NUM_MMA_WARPS: cutlass.Constexpr = self.num_mma_warps
        bM_total = 16 * NUM_MMA_WARPS  # 64
        num_stages = self.num_stages
        tidx, _, _ = cute.arch.thread_idx()
        bid_m, bid_n, _ = cute.arch.block_idx()
        warp_idx = tidx // 32
        ...
        # Global tiles: A uses bM_total (64 rows), B uses bN (8 rows)
        cta_tiler_A = (bM_total, bN, bK)
        cta_tiler_B = (bM_total, bN, bK)
        coord = (bid_m, bid_n, None)
        gA = cute.local_tile(mA, tiler=cta_tiler_A, coord=coord, proj=(1, None, 1))
        gB = cute.local_tile(mB, tiler=cta_tiler_B, coord=coord, proj=(None, 1, 1))
        gA = cute.make_tensor(gA.iterator.align(16), gA.layout)
        gB = cute.make_tensor(gB.iterator.align(16), gB.layout)

        mcA = cute.make_identity_tensor(mA.layout.shape)
        mcB = cute.make_identity_tensor(mB.layout.shape)
        cA = cute.local_tile(mcA, tiler=cta_tiler_A, coord=coord, proj=(1, None, 1))
        cB = cute.local_tile(mcB, tiler=cta_tiler_B, coord=coord, proj=(None, 1, 1))
        ...
        if is_dma:
            ...
            producer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, num_stages)
            # First tile: PDL overlap
            mainloop_pipeline.producer_acquire(producer_state)
            cute.copy(tiled_copy_B, tBgB[None, None, None, 0], tBsB[None, None, None, producer_state.index], pred=tBpB)
            cute.arch.griddepcontrol_wait()
            cute.copy(tiled_copy_A, tAgA[None, None, None, 0], tAsA[None, None, None, producer_state.index], pred=tApA)
            mainloop_pipeline.producer_commit(producer_state)
            producer_state.advance()
            ...
            # Each warp gets its own 16-row sub-tile of sA via local_tile
            sA_warp_tiler = (16, cute.size(sA, mode=[1]), cute.size(sA, mode=[2]))
            sA_warp = cute.local_tile(sA, tiler=sA_warp_tiler, coord=(mma_warp_idx, 0, 0))

            ab_width = mA.element_type.width
            sA_warp_view = cute.recast_tensor(sA_warp, cutlass.BFloat16)
            sB_view = cute.recast_tensor(sB, cutlass.BFloat16)

            thr_mma = tiled_mma.get_slice(lane_id)
            tCsA = thr_mma.partition_A(sA_warp_view)
            tCsB = thr_mma.partition_B(sB_view)

            # Output: each warp writes to its own M-sub-tile region
            # gC_warp covers output rows [bid_m*64 + warp*16 : bid_m*64 + (warp+1)*16]
            cta_tiler_C = (16, bN, bK)
            warp_bid_m = bid_m * NUM_MMA_WARPS + mma_warp_idx
            coord_c = (warp_bid_m, bid_n, None)
            gC_warp = cute.local_tile(mC, tiler=cta_tiler_C, coord=coord_c, proj=(1, 1, None))
            tCgC = thr_mma.partition_C(gC_warp)
            ...
            atom_s2r_A = cute.make_copy_atom(cute.nvgpu.warp.LdMatrix8x8x16bOp(False, 4), cutlass.BFloat16)
            atom_s2r_B = cute.make_copy_atom(cute.nvgpu.warp.LdMatrix8x8x16bOp(False, 4), cutlass.BFloat16)
            tiled_s2r_A = cute.make_tiled_copy_A(atom_s2r_A, tiled_mma)
            tiled_s2r_B = cute.make_tiled_copy_B(atom_s2r_B, tiled_mma)
            thr_s2r_A = tiled_s2r_A.get_slice(lane_id)
            thr_s2r_B = tiled_s2r_B.get_slice(lane_id)
            tCsA_v = thr_s2r_A.partition_S(sA_warp_view)
            tCrA_v = thr_s2r_A.retile(tCrA)
            tCsB_v = thr_s2r_B.partition_S(sB_view)
            tCrB_v = thr_s2r_B.retile(tCrB)

            num_k_block = cute.size(tCrA, mode=[2])
            consumer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, num_stages)

            # Main K loop: each warp processes ALL k-tiles (no K-parallelism)
            for k_tile in range(k_tile_count):
                mainloop_pipeline.consumer_wait(consumer_state)
                tCsA_p = tCsA_v[None, None, None, consumer_state.index]
                tCsB_p = tCsB_v[None, None, None, consumer_state.index]
                if not self.is_fp8:
                    for ki in range(num_k_block):
                        cute.copy(tiled_s2r_A, tCsA_p[None, None, ki], tCrA_v[None, None, 0])
                        cute.copy(tiled_s2r_B, tCsB_p[None, None, ki], tCrB_v[None, None, 0])
                        cute.gemm(tiled_mma, tCrC, tCrA[None, None, 0], tCrB[None, None, 0], tCrC)
                else:
                    a_s = tCrA[None, None, 0]; b_s = tCrB[None, None, 0]
                    c0=tCrC[0];c1=tCrC[1];c2=tCrC[2];c3=tCrC[3];c4=tCrC[4];c5=tCrC[5];c6=tCrC[6];c7=tCrC[7]
            ...
            # Epilogue: write accumulators to smem via MMA partition, then to global
            # Each warp uses its own smem region for the m16xn8 output
            smem_c_ptr = cute.arch.alloc_smem(cutlass.Float32, 16 * bN * NUM_MMA_WARPS, alignment=16)
            smem_warp_c = cute.make_tensor(smem_c_ptr + mma_warp_idx * 16 * bN, cute.make_layout((16, bN), stride=(bN, 1)))
            tCsC_w = thr_mma.partition_C(smem_warp_c)
            cute.autovec_copy(tCrC, tCsC_w)
            cute.arch.sync_threads()

            # Scale + write to global — each warp writes its own 16xbN tile
            num_warp_elems: cutlass.Constexpr = 16 * bN
            warp_elems_per_thread: cutlass.Constexpr = num_warp_elems // 32
            global_m_base = bid_m * bM_total + mma_warp_idx * 16
            for ei in cutlass.range_constexpr(warp_elems_per_thread):
                idx = ei * 32 + lane_id
                m = idx // bN
                n = idx % bN
                global_m = global_m_base + m
                global_n = bid_n * bN + n
                if global_m < M_out:
                    if global_n < N_out:
                        s_p = smem_c_ptr + mma_warp_idx * 16 * bN + m * bN + n
                        s_t = cute.make_tensor(s_p, cute.make_layout((1,)))
                        s_r = cute.make_rmem_tensor((1,), cutlass.Float32)
                        cute.autovec_copy(s_t, s_r)
                        val = s_r[0] * scale
                        out_p = (mC.iterator + global_m * N_out + global_n).align(2)
                        out_t = cute.make_tensor(out_p, cute.make_layout((1,)))
                        out_r = cute.make_rmem_tensor((1,), self.out_dtype)
                        out_r[0] = val.to(self.out_dtype)
                        out_t[0] = out_r[0]
    ...
    @cute.jit
    def call_splitk_atomic(self, mA: cute.Tensor, mB: cute.Tensor,
                           mC: cute.Tensor, stream: CUstream, scale: float = 1.0):
        """Split-K with global atomic reduction (no cluster barriers)."""
        bM, bN, bK = self.tile_m, self.tile_n, self.tile_k
        copy_bits = 128
        sA_layout = self._make_smem_layout_AB(mA.element_type, copy_bits, (bM, bK, self.num_stages))
        sB_layout = self._make_smem_layout_AB(mB.element_type, copy_bits, (bN, bK, self.num_stages))
        ...
        grid_n = cute.ceil_div(cute.size(mC, mode=[1]), bN)
        scale_val = cutlass.Float32(scale)
        # NO cluster — each CTA is independent
        self.kernel_splitk_atomic(mA, mB, mC, scale_val, sA_layout, sB_layout,
    ...
    @cute.kernel
    def kernel_splitk_atomic(self, mA, mB, mC, scale: cutlass.Float32,
        sA_layout: cute.ComposedLayout, sB_layout: cute.ComposedLayout,
        tiled_copy_A: cute.TiledCopy, tiled_copy_B: cute.TiledCopy,
        tiled_mma: cute.TiledMma):
        """Split-K kernel with global atomic reduction — no cluster barriers."""
        bM, bN, bK = self.tile_m, self.tile_n, self.tile_k
        num_stages = self.num_stages
        tidx, _, _ = cute.arch.thread_idx()
        bid_m, bid_n, bid_z = cute.arch.block_idx()
        ...
        if is_dma:
            ...
            NUM_MMA_WARPS: cutlass.Constexpr = self.num_mma_warps
            thr_mma = tiled_mma.get_slice(lane_id)
            sA_view = cute.recast_tensor(sA, cutlass.BFloat16)
            sB_view = cute.recast_tensor(sB, cutlass.BFloat16)
            tCsA = thr_mma.partition_A(sA_view); tCsB = thr_mma.partition_B(sB_view)
            tCgC = thr_mma.partition_C(gC)
            tCrA = tiled_mma.make_fragment_A(tCsA[None, None, None, 0])
            tCrB = tiled_mma.make_fragment_B(tCsB[None, None, None, 0])
            tCrC = tiled_mma.make_fragment_C(tCgC); tCrC.fill(0.0)
            ...
            tiled_s2r_B = cute.make_tiled_copy_B(atom_s2r_B, tiled_mma)
            thr_s2r_A = tiled_s2r_A.get_slice(lane_id); thr_s2r_B = tiled_s2r_B.get_slice(lane_id)
            tCsA_v = thr_s2r_A.partition_S(sA_view); tCrA_v = thr_s2r_A.retile(tCrA)
            tCsB_v = thr_s2r_B.partition_S(sB_view); tCrB_v = thr_s2r_B.retile(tCrB)
            num_k_block = cute.size(tCrA, mode=[2])
            K_PER_WARP: cutlass.Constexpr = num_k_block // NUM_MMA_WARPS
            consumer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, num_stages)
            for local_k in range(my_tiles):
            ...
            # === WORKSPACE WRITE EPILOGUE (no reduction) ===
            # Each CTA writes its scaled partial to mC at its bid_z slice
            # mC layout: [split_k * M_tiles, N_tiles] — bid_z offsets by M_tiles
            smem_red_ptr = cute.arch.alloc_smem(cutlass.Float32, bM * bN * NUM_MMA_WARPS, alignment=16)
            smem_warp = cute.make_tensor(smem_red_ptr + mma_warp_idx * bM * bN, cute.make_layout((bM, bN), stride=(bN, 1)))
            tCsC_partial = thr_mma.partition_C(smem_warp)
            cute.autovec_copy(tCrC, tCsC_partial)
                ...
                if global_m < M_out:
                    if global_n < N_out:
                        ...
                        for w in cutlass.range_constexpr(NUM_MMA_WARPS):
                            ...
                            total = total + r[0]
                        total = total * scale
                        # Write to workspace: offset by bid_z * M_logical
                        M_logical = M_out // self.split_k
                        ws_m = bid_z * M_logical + global_m
                        out_p = (mC.iterator + ws_m * N_out + global_n).align(2)
                        out_t = cute.make_tensor(out_p, cute.make_layout((1,)))
                        out_r = cute.make_rmem_tensor((1,), self.out_dtype)
