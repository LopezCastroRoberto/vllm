...
def _warm_prefill_chunk_metadata_kernel(
    ...
    num_reqs = 2
    query_len = _SPARSE_METADATA_WARMUP_PREFILL_TOKENS
...
def _warm_combine_topk_swa_indices_kernel(
    ...
    num_tokens = _clamp_warmup_tokens(
        num_tokens, _SPARSE_METADATA_WARMUP_PREFILL_TOKENS
    )
    if num_tokens <= 0:
        return
...
def sparse_mla_triton_warmup_if_needed(worker: "Worker") -> None:
    ...
    num_tokens = _clamp_warmup_tokens(_SPARSE_METADATA_WARMUP_PREFILL_TOKENS, max_tokens)
    if num_tokens <= 0:
        return
