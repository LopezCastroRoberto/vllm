...
def test_layer_norm_fwd_warmup_keys_cover_qwen_gdn(
    ...
    group_size = 128
    max_num_tokens = 512
    warmup_keys = layer_norm_fwd_kernel.get_warmup_keys(
    ...
    expected_specializations = {
        (
            triton_scalar_specialization_rep(num_tokens * rows_per_token),
            calc_rows_per_block(num_tokens * rows_per_token, device),
        )
        for num_tokens in range(1, max_num_tokens + 1)
    }
    assert len(warmup_keys) == len(expected_specializations)
